...
tests_to_try = ['test_6']                     # executed tests
...
class SeriesPage(BasePage):
    ...
    def get_nth_season_name(self, n):
        ...
        return self.driver.find_elements(By.XPATH,"//div[@id='seasons-official']/table/tbody/tr/td[contains(.,'Season ')]/a")[n].text.strip()
    
    def click_nth_season(self,n):
        """Click the n-th main season for the series (the extra seasons are excluded; seasons count starts from 0)"""
        self.driver.find_elements(By.XPATH,"//div[@id='seasons-official']/table/tbody/tr/td[contains(.,'Season ')]/a")[n].click()
    ...
    def click_nth_actor_elem(self,n):
        """Click the webelement of the n-th actor (starting from 1)"""
        self.driver.find_element(By.CSS_SELECTOR, "#people-actor > div > div:nth-of-type({0}) h3".format(n)).click()
        
    def get_nth_actor_name(self,n):
        """Get the name of the n-th actor (starting from 1)"""
        nth_actor_elem = self.driver.find_element(By.CSS_SELECTOR, "#people-actor > div > div:nth-of-type({0}) h3".format(n))
        return nth_actor_elem.text.split('\nas ')[0].strip()
...
class TheTVDB_tests(unittest.TestCase):
    ...
    def test_4(self):
        ...
        for n in range(1,test4_n_actors+1): # iterate on each actor
            actor_name = seriesPage.get_nth_actor_name(n)
            seriesPage.click_nth_actor_elem(n)
            CharacterPage(self.driver).click_actor()
            
            actor_gender = ActorPage(self.driver).get_gender()
